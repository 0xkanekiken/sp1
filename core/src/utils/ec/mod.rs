...
impl<E: EllipticCurve> AffinePoint<E> {
    ...
    pub fn to_words_le(&self) -> [u32; E::NB_LIMBS] {
        let mut x_bytes = self.x.to_bytes_le();
        x_bytes.resize(E::NB_LIMBS * 4 / 2, 0u8);
        let mut y_bytes = self.y.to_bytes_le();
        y_bytes.resize(E::NB_LIMBS * 4 / 2, 0u8);

        let mut words = [0u32; E::NB_LIMBS];
        for i in 0..E::NB_LIMBS / 2 {
            ...
            words[i + E::NB_LIMBS / 2] = u32::from_le_bytes([
                y_bytes[i * 4],
                y_bytes[i * 4 + 1],
                y_bytes[i * 4 + 2],
                y_bytes[i * 4 + 3],
